...
function compileBicepToJson(filePath: string): any {
  const bicep = which("bicep") || null;
  const az = which("az") || null;
  if (bicep) {
    const r = spawnSync(bicep, ["build", filePath, "--stdout"], { encoding: "utf8" });
    if (r.status !== 0) throw new Error(`bicep build failed: ${r.stderr || r.stdout}`);
    return JSON.parse(r.stdout || "{}");
  }
  if (az) {
    const r = spawnSync(az, ["bicep", "build", "--file", filePath, "--stdout"], { encoding: "utf8" });
    if (r.status !== 0) throw new Error(`az bicep build failed: ${r.stderr || r.stdout}`);
    return JSON.parse(r.stdout || "{}");
  }
  throw new Error("No Bicep compiler found. Install 'bicep' or 'az' CLI.");
}

function cloneGitRepo(repo: string, ref?: string): string {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "bicep-repo-"));
  const git = which("git");
  if (!git) throw new Error("git is required to fetch templates from a repo");
  let r = spawnSync(git, ["clone", "--depth", "1", repo, tmp], { encoding: "utf8" });
  if (r.status !== 0) throw new Error(`git clone failed: ${r.stderr || r.stdout}`);
  if (ref) {
    r = spawnSync(git, ["-C", tmp, "fetch", "--depth", "1", "origin", ref], { encoding: "utf8" });
    if (r.status === 0) r = spawnSync(git, ["-C", tmp, "checkout", ref], { encoding: "utf8" });
    if (r.status !== 0) throw new Error(`git checkout ${ref} failed: ${r.stderr || r.stdout}`);
  }
  return tmp;
}

function resolveTemplateFromSource(a: any): { baseDir: string; templatePath: string; template: any; parameters?: Record<string, { value: any }> } {
  let baseDir: string;
  ...
  let paramsObj: Record<string, any> | undefined = a.parameters ? { ...a.parameters } : undefined;
  if (a.parametersFile) {
...
export function makeAzureDeployTools(opts: MakeAzureToolsOptions & { namespace?: string }): ToolDef[] {
  const { clients, namespace = "azure." } = opts;
  const n = (s: string) => `${namespace}${s}`;

  // RG scope
  const deploy_bicep: ToolDef = {
    name: n("deploy_bicep"),
    description:
      "Deploy a Bicep/ARM template to a Resource Group. Source can be a local file or a Git repo. Supports optional parameters and What-If.",
    inputSchema: z
      .object({
    ...
    handler: async (a: any) => {
      try {
        ...
        const properties = { mode: a.mode, template, parameters } as const;
        const result = await clients.deployments.deployToResourceGroup(
          a.resourceGroupName,
          a.deploymentName,
          properties,
          { whatIf: a.whatIf }
  ...
  // Subscription scope
  const deploy_bicep_subscription: ToolDef = {
    ...
    handler: async (a: any) => {
      try {
        const { template, parameters } = resolveTemplateFromSource(a);
        const properties = { location: a.location, mode: a.mode, template, parameters } as const;
        const result = await clients.deployments.deployToSubscription(
          a.deploymentName,
          properties,
          { whatIf: a.whatIf }
        );
  ...
  const deploy_bicep_management_group: ToolDef = {
    name: n("deploy_bicep_management_group"),
    description:
      "Deploy a Bicep/ARM template at **management group scope** (requires managementGroupId and location).",
    inputSchema: z
      .object({
    ...
    handler: async (a: any) => {
      try {
        const { template, parameters } = resolveTemplateFromSource(a);
        const properties = { location: a.location, mode: a.mode, template, parameters } as const;
        const result = await clients.deployments.deployToManagementGroup(
          a.managementGroupId,
          a.deploymentName,
          properties,
          { whatIf: a.whatIf }
