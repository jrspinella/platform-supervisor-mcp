...
export async function evaluate(toolFq, args) {
    const fq = TOOL_MAP[toolFq] || toolFq;
    const az = ensureLoaded()?.azure || {};
    const p = az?.create_resource_group;
    if (fq === "azure.create_resource_group" && p) {
        ...
        if (Array.isArray(p.deny_contains) && p.deny_contains.some((w) => name.includes(w))) {
            reasons.push("name contains denied token(s)");
        }
        if (p.deny_regex) {
            try {
                if (new RegExp(p.deny_regex).test(name))
                    reasons.push("name matches deny_regex");
            }
            catch { /* ignore invalid regex */ }
        }
        if (p.name_regex) {
            try {
                if (!new RegExp(p.name_regex).test(name))
                    reasons.push(`name does not match required pattern ${p.name_regex}`);
            }
            catch { /* ignore invalid regex */ }
        }
        ...
        if (Array.isArray(p.require_tags)) {
            const missing = p.require_tags.filter((k) => !(k in tags));
            if (missing.length)
                reasons.push(`missing required tag(s): ${missing.join(", ")}`);
        }
        if (p.suggest_name)
            suggestions.push({ title: "Suggested name", text: p.suggest_name });
        if (p.suggest_region)
            suggestions.push({ title: "Suggested region", text: p.suggest_region });
        if (p.suggest_tags) {
