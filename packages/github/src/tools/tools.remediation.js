...
function suggestNextStepsForRepo(steps, results) {
    const tips = [];
    ...
    if (steps.some((s) => s.action === "repos.updateBranchProtection") &&
        !steps.some((s) => Array.isArray(s?.args?.requiredStatusChecksContexts) &&
    ...
    if (steps.some((s) => s.action === "actions.upsertEnvironment") &&
...
function renderTextReport(title, perRepo) {
    const lines = [title];
    for (const [repo, r] of Object.entries(perRepo)) {
        ...
        const sevLine = ["high", "medium", "low", "info", "unknown"]
            .map((k) => `${k}:${sev[k] ?? 0}`)
            .join(" ");
        lines.push(`- ${repo} — planned:${r.plannedSteps} applied:${a} failed:${f}` +
...
function planFromFindings(findings, owner, repo, defaults) {
    const steps = [];
    const want = defaults || {};
...
export function makeGithubRemediationTools(opts) {
    const { clients, namespace = "github." } = opts;
    const n = (s) => `${namespace}${s}`;
    const remediate_repo_baseline = {
        name: n("remediate_repo_baseline"),
        description: "Apply opinionated fixes for repo/pipeline baseline findings. Best-effort; ignores unsupported features.",
        inputSchema: z
            .object({
            owner: z.string(),
            repo: z.string(),
                ...
                for (const s of steps) {
                    try {
                        if (s.action === "repos.updateVisibility") {
                            const out = clients.repos.update
                                ? await clients.repos.update(a.owner, a.repo, {
                            ...
                            const out = await clients.repos.enableSecurityFeatures(a.owner, a.repo, {
                            ...
                            const bp = s.args;
                            const out = await clients.repos.updateBranchProtection(a.owner, a.repo, {
                            ...
                            const out = clients.actions.upsertEnvironment
                                ? await clients.actions.upsertEnvironment(a.owner, a.repo, s.args.environment, s.args)
                                : { ok: true, note: "upsertEnvironment not implemented in client" };
                            results.push({ action: s.action, ok: true, result: out });
                        }
                        else if (s.action === "actions.setPermissions") {
                            const out = clients.actions.setPermissions
                                ? await clients.actions.setPermissions(a.owner, a.repo, s.args.policy)
    ...
    const remediate_org_repos_baseline = {
        name: n("remediate_org_repos_baseline"),
        description: "Batch remediate common baseline findings across an org. Supports dry-run planning and returns a per-repo report.",
        inputSchema: z
            .object({
            org: z.string(),
        ...
        handler: async (a) => {
            try {
                ...
                for (const r of slice) {
                    ...
                    const fs = [];
                    if (r.private !== true)
                        fs.push({ code: "REPO_NOT_PRIVATE", severity: "high" });
                    const sa = r.security_and_analysis || {};
                    if (sa.secret_scanning?.status !== "enabled")
                        fs.push({ code: "REPO_SECRET_SCANNING_DISABLED", severity: "high" });
                    if (sa.secret_scanning_push_protection?.status !== "enabled")
                        fs.push({ code: "REPO_SECRET_PUSH_PROTECTION_DISABLED", severity: "medium" });
                    ...
                    for (const s of steps) {
                        try {
                            if (s.action === "repos.enableSecurityFeatures") {
                                ...
                                const out = clients.actions.upsertEnvironment?.(a.org, repoName, s.args.environment, s.args) ?? {
                                    ok: true,
                                ...
                                const out = clients.actions.setPermissions?.(a.org, repoName, s.args.policy) ?? {
                                    ok: true,
    ...
    const autofix_org_findings = {
        name: n("autofix_org_findings"),
        description: "Auto-fix selected finding codes from github.scan_org_repos_baseline output (plan/apply) and return a remediation report.",
        inputSchema: z
            .object({
            org: z.string(),
            findings: z
        ...
        handler: async (a) => {
            try {
                ...
                for (const f of a.findings) {
                    ...
                    if (!allow.has(code))
                        continue;
                    const owner = f?.meta?.owner || a.org;
                    const repo = f?.meta?.repo;
                    if (!repo)
                        continue;
                ...
                for (const [key, fs] of byRepo) {
                    ...
                    for (const s of steps) {
                        try {
                            if (s.action === "repos.updateVisibility") {
                                const res = clients.repos.update
                                    ? await clients.repos.update(owner, repo, { private: s.args.visibility === "private" })
                                    : { ok: false, note: "repos.update not available in client" };
                                ...
                                const res = clients.actions.upsertEnvironment
                                    ? await clients.actions.upsertEnvironment(owner, repo, s.args.environment, s.args)
                                    : { ok: true, note: "upsertEnvironment not implemented in client" };
                                out.push({ action: s.action, ok: true, result: res });
                            }
                            else if (s.action === "actions.setPermissions") {
                                const res = clients.actions.setPermissions
                                    ? await clients.actions.setPermissions(owner, repo, s.args.policy)
                                    : { ok: true, note: "setPermissions not implemented in client" };
                                out.push({ action: s.action, ok: true, result: res });
                            }
                            else {
    ...
    const autofix_repo_findings = {
        name: n("autofix_repo_findings"),
        description: "Auto-fix selected finding codes for a single repo (plan/apply) and return a remediation report.",
        inputSchema: z
            .object({
            owner: z.string(),
            repo: z.string(),
        ...
        handler: async (a) => {
            try {
                ...
                const allow = new Set((a.codes && a.codes.length ? a.codes : Array.from(SAFE_DEFAULT_CODES)).map((c) => c.toUpperCase()));
                const repoFindings = a.findings.filter((f) => allow.has(String(f.code).toUpperCase()));
                ...
                for (const s of steps) {
                    try {
                        if (s.action === "repos.updateVisibility") {
                            ...
                            const res = await clients.repos.enableSecurityFeatures(a.owner, a.repo, {
                                enableDependabot: !!s.args.enableDependabot,
                                enableAdvancedSecurity: !!s.args.enableAdvancedSecurity,
                            });
                            out.push({ action: s.action, ok: true, result: res });
                            ...
                            const bp = s.args;
                            const res = await clients.repos.updateBranchProtection(a.owner, a.repo, {
                                branch: bp.branch || "main",
                                requiredApprovingReviewCount: bp.requiredApprovingReviewCount ?? 2,
                                requireCodeOwnerReviews: bp.requireCodeOwnerReviews ?? true,
                                dismissStaleReviews: bp.dismissStaleReviews ?? true,
