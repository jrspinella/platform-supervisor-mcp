...
export function makeGithubScanTools(opts) {
    ...
    async function scanRepo(owner, repo, profile) {
        const name = repo.name;
        const defBranch = repo.default_branch;
        const findings = [];
        if (repo.private !== true)
            findings.push({ code: "REPO_NOT_PRIVATE", severity: "high", meta: { owner, repo: name } });
        ...
        if (!secretScan)
            findings.push({ code: "REPO_SECRET_SCANNING_DISABLED", severity: "high", meta: { repo: name } });
        if (!secretPP)
            findings.push({ code: "REPO_SECRET_PUSH_PROTECTION_DISABLED", severity: "medium", meta: { repo: name } });
        if (!depUpdates)
            findings.push({ code: "REPO_DEPENDABOT_UPDATES_DISABLED", severity: "low", meta: { repo: name } });
        try {
            const bp = await clients.repos.getBranchProtection(owner, name, defBranch);
            ...
            if (!statusChecks)
                findings.push({ code: "REPO_STATUS_CHECKS_MISSING", severity: "medium", meta: { repo: name } });
        }
        catch {
    ...
    async function scanPipeline(owner, repoName, profile) {
        const findings = [];
        try {
            const envs = await clients.actions.listEnvironments(owner, repoName);
            for (const e of envs) {
                const reviewers = Array.isArray(e?.protection_rules)
                    ? e.protection_rules.flatMap((r) => r?.reviewers ?? [])
                    : [];
                if (!reviewers.length)
                    findings.push({ code: "PIPELINE_ENV_NO_REVIEWERS", severity: "medium", meta: { repo: repoName, environment: e?.name } });
            }
        }
        catch {
            // ignore env errors
    ...
    const scan_org_repos_baseline = {
        ...
        handler: async (a) => {
            try {
                await ensureAto();
                const kinds = new Set((a.include && a.include.length ? a.include : ["repo", "pipeline"]));
                for (const k of a.exclude || [])
                    kinds.delete(k);
